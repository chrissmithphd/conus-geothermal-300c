...
# Area in km² by depth bin — read from the depth analysis output, which is the
# single source of truth. Regenerated by calculate_depth_to_300c.py; never hardcode.
AREA_STATS_FILE = "data/processed/depth_bin_area_stats.csv"
_area_df = pd.read_csv(AREA_STATS_FILE)
AREA_BY_DEPTH = dict(zip(_area_df["bin"], _area_df["area_km2"].round().astype(int)))
